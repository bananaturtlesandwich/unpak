...
static void
LznaNibbleModel_Init (LznaNibbleModel *d)
{
  *d = lzna_initializer_4bit;
}

static void
Lzna3bitModel_Init (Lzna3bitModel *d)
{
  *d = lzna_initializer_3bit;
}

static void
LznaNibbleModel_InitN (LznaNibbleModel *d, int32_t n)
{
  do
...
static void
LznaLiteralModel_InitN (LznaLiteralModel *d, int32_t n)
{
  do
    {
...
static void
LznaShortLengthRecentModel_InitN (LznaShortLengthRecentModel *d, int32_t n)
{
  do
    {
...
static void
LznaNearDistModel_Init (LznaNearDistModel *d, int32_t n)
{
  int32_t i;
  do
    {
...
static void
LznaLowBitsDistanceModel_Init (LznaLowBitsDistanceModel *d, int32_t n)
{
  do
    {
      d->v = 0x2000;
...
static void
LznaFarDistModel_Init (LznaFarDistModel *d)
{
  int32_t i;
  ...
  if (n != 0)
    {
      ...
      if (n != 1)
        {
          hi = (hi << 1)
               + LznaRead1Bit (tab, &lut->far_distance.third[hi - 2][n - 1],
                               14, 6);
          if (n != 2)
  ...
  if (nb != 0)
    {
      ...
      if (nb != 1)
        {
          hi = (hi << 1)
               + LznaRead1Bit (tab, &model->third[hi - 2][nb - 1], 14, 6);
          if (nb != 2)
...
static void
LznaPreprocessMatchHistory (LznaState *lut)
{
  ...
  if (dst_offs == 0)
    {
      if (LznaReadBit (&tab))
      ...
      else
        {
          ...
          x = (x << 4)
              + LznaReadNibble (&tab, (x != 0) ? &model->nomatch[x]
                                               : &model->lower[0]);
        }
      *dst++ = x;
      dst_offs += 1;
  ...
  while (dst < dst_end)
    {
      ...
      if (LznaRead1Bit (&tab, &lut->is_literal[(dst_offs & 7) + 8 * state], 13,
              ...
              if (x == 1)
                {
                  ...
                  length = 3
                           + LznaRead1Bit (
                               &tab, &lut->short_length[state][dst_offs & 3],
                               14, 4);
                  dist = LznaReadNearDistance (&tab, lut,
                                               &lut->near_dist[length - 3]);
                  dst[0] = (dst - dist)[0];
                  dst[1] = (dst - dist)[1];
                  dst[2] = (dst - dist)[2];
                  dst[3] = (dst - dist)[3];
                ...
                {
                  // Copy count 13-
                  length = LznaReadLength (&tab, &lut->long_length, dst_offs)
                           + 13;
                  dist = LznaReadFarDistance (&tab, lut);
                  if (dist >= 8)
      ...
      else
        {
          ...
          x = (x << 4)
              + LznaReadNibble (&tab, ((match_val >> 4) != x)
                                          ? &model->nomatch[x]
