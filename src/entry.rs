    ...
    pub fn read<W: io::Write>(
        ...
        match self.compression.and_then(|i| compression.get(i)) {
            ...
            Some(Compression::Oodle) => {
                let mut scratch = vec![0; self.block_uncompressed as usize];
                let state = [0; 0x6C000].as_mut_ptr().cast();
                unsafe { ffi::LZNA_InitLookup(state) };
                let block_count = blocks.len();
                for (i, block) in blocks.into_iter().enumerate() {
                    let out_len = match block_count == 1 {
                        true => self.uncompressed,
                        false => self
                            .block_uncompressed
                            .min(self.uncompressed - (i as u64 + 1) * self.block_uncompressed),
                    } as usize;
                    let data = &data[block];
                    unsafe {
                        ffi::LZNA_DecodeQuantum(
                            scratch.as_mut_ptr(),
                            scratch.as_mut_ptr().add(out_len),
                            scratch.as_mut_ptr(),
                            data.as_ptr(),
